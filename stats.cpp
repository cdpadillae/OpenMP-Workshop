// stats.cpp
//Authors: DAZA DANIELA-PADILLA CINDY
//We will be using an "alternative" standard deviation formula 
#include <omp.h>
#include <iostream>
#include <cmath>
#include <vector>
...
void stats(const std::vector<double> & array, double & mean, double & sigma)
{
  ...
  double suma = 0.0;
  double square = 0.0;
#pragma omp parallel for reduction(+:suma, square)
  for(int ii = 0; ii < N; ii++) {
    suma += array[ii];
    square += array[ii]*array[ii];
  }
  mean = suma/N;
  sigma = std::sqrt(square/N-mean*mean);
}
